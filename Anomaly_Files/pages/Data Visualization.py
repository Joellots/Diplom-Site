...
if plot_type == 'Гистограмма':
    ...
    if st.sidebar.button('Построить'):
        st.markdown("**Гистограммы:** Используются для отображения распределения числовых данных. Позволяют увидеть, как часто встречаются значения в определенных диапазонах.")
        fig, ax = plt.subplots()
    ...
    plt.rcParams["figure.figsize"] = (28, 10)
    if st.sidebar.button('Построить'):
        st.markdown("**Столбчатые диаграммы:** Применяются для визуализации частоты категориальных данных. Помогают сравнивать количество элементов в различных категориях.")
        fig, ax = plt.subplots()
    ...
    if st.sidebar.button('Построить'):
        st.markdown("**Тепловые карты корреляций:** Используются для отображения взаимосвязей между числовыми признаками. Позволяют быстро определить, какие признаки имеют сильные корреляции.")
